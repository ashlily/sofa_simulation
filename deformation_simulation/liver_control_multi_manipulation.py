...
class TutorialForceFieldLiverFEM (Sofa.PythonScriptController):
    ...
    #fp_index = [82,88, 92, 93, 94, 96, 97, 101, 104, 105, 110]
    fp_index = [85,91,96,97,98,99,101,104,105,110]
    manipulation_flag = 0
    ...
    def onBeginAnimationStep(self, deltaTime):
        ...
        try:
            ...
            manipulation_dof = 3 * len(self.fp_index)
            ...
            if self.data == 'Hello, world':
               ...
               print "cmd_split is:"
               print cmd_split
               print "len(cmd_split) is:"
               print len(cmd_split)
               '''
               if len(cmd_split) < manipulation_dof:
                  cmd_f =[0 for i in range(manipulation_dof)]
            ...
            if self.manipulation_flag >= 10:
               ...
               file = open("/home/ashily/sofa/v17.06/src/applications/plugins/SofaPython/examples/deformation_simulation/revise/manipulation_p.txt","a")
               '''
               for id in range(5):
                   file.write(str(id)+" "+str(self.LiverFEM.getObject('dofs').position[id])+"\n")
               '''
               file.write(str(self.LiverFEM.getObject('dofs').position[100])+"\n")
               file.close()
               ...
               #self.LiverFEM.getObject('PartialLinearMovementConstraint').findData('movements').value = [0.5*math.sin(cT)+cmd_f[0], 0.5*math.cos(cT)+cmd_f[1], 0+10*cmd_f[2]]
               
               amp = 0.8
               self.mp1.findData('movements').value = [amp*math.sin(cT), amp*math.cos(cT), 0]
               self.mp2.findData('movements').value = [amp*math.sin(cT), amp*math.cos(cT), 0]
               self.mp3.findData('movements').value = [amp*math.sin(cT), amp*math.cos(cT), 0]
               self.mp4.findData('movements').value = [amp*math.sin(cT), amp*math.cos(cT), 0]
               self.mp5.findData('movements').value = [amp*math.sin(cT), amp*math.cos(cT), 0]
               self.mp6.findData('movements').value = [amp*math.sin(cT), amp*math.cos(cT), 0]
               self.mp7.findData('movements').value = [amp*math.sin(cT), amp*math.cos(cT), 0]
               self.mp8.findData('movements').value = [amp*math.sin(cT), amp*math.cos(cT), 0]
               self.mp9.findData('movements').value = [amp*math.sin(cT), amp*math.cos(cT), 0]
               self.mp10.findData('movements').value = [amp*math.sin(cT), amp*math.cos(cT), 0]
               ...
               #self.LiverFEM.getObject('PartialLinearMovementConstraint').findData('movements').value = [-0.5, 0, 0]
